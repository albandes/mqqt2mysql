...
class mqtt2mysql
{
     ...
     * @return void
     */
    public function saveTopic($sql,$aValues)
    {

        $lastInsertId = $this->_db->insert($sql, $aValues);
        if(is_null($lastInsertId)) {
            $this->_applogger->error('Error saving to database! ',['sql'=>$sql, 'values'=> $aValues]);    
            return;                     
        }

        $this->_applogger->info('Saved in the database ',['lastInsertId'=>$lastInsertId, 'values'=> $aValues]); 
